/*
 * View model for E3S1PROFORKBYTT Thumbnails
 *
 * Author: ThomasToka
 * License: AGPLv3
 */
		...
		self.onBeforeBinding = function() {
            ...
            if(self.uploadmanagerViewModel) {
                $("#uploadmanager_template_machinecode").text(function () {
					let uploadmanager_thumbnail_template = '<i class="fa-regular fa-file-lines" data-bind="visible: ($data.thumbnail_src == \'e3s1proforkbyttthumbnails\' && $root.settings.settings.plugins.e3s1proforkbyttthumbnails.inline_thumbnail_uploadmanager() == false)"></i><div class="inline_prusa_thumbnail" ' +
												'data-bind="if: ($data.thumbnail_src == \'e3s1proforkbyttthumbnails\' && $root.settings.settings.plugins.e3s1proforkbyttthumbnails.inline_thumbnail_uploadmanager() == true)">' +
                                                '<img data-bind="attr: {src: $data.thumbnail}, ' +
												'visible: ($data.thumbnail_src == \'e3s1proforkbyttthumbnails\' && $root.settings.settings.plugins.e3s1proforkbyttthumbnails.inline_thumbnail_uploadmanager() == true), ' +
												'click: function() { if ($root.loginState.isUser() && !($(\'html\').attr(\'id\') === \'touch\')) { $root.files.e3s1proforkbyttthumbnails_open_thumbnail($data) } else { return; } },' +
                                                'style="display: none;"/></div>';
